...
if ( ! class_exists( 'MPC_Admin_Field' ) ) {
	...
	class MPC_Admin_Field {
		...
		private static function render_field() {
			$classes = array();
			if ( isset( self::$field['class'] ) ) {
				$classes[] = self::$field['class'];
			}
			if ( isset( self::$field['pro'] ) && empty( self::$pro_state ) ) {
				$classes[] = 'mpcex-disabled';
			}
			...
			<div class="mpcdp_settings_option_field mpcdp_settings_option_field_text col-md-6">
				<div class="switch-field">
					...
					foreach ( $field['options'] as $opt_value => $opt_label ) {
						self::render_field_radio_option(
							$field,
							array(
								'value'   => $opt_value,
								'label'   => $opt_label,
		...
		/**
		 * Display the followup fields
		 *
		 * @param array  $field   Field data.
		 * @param string $display Is it display block or none.
		 */
