...
function wmc_admin_options( $settings, $current_section ) {
    ...
    if ( $current_section == 'wmc' ) {
        ...
        $wmc_settings[] = array(
            ...
            'desc' => __( 'For quick support, contact us <a href="https://webfixlab.com/#contact-us" target="_blank">here.</a>', 'mpc' ),
...
function mpc_go_to_designated_page( $url = '' ){
	...
	// wp_safe_redirect( wc_get_cart_url() . '?wcpls=' . $_SERVER['REQUEST_URI'] );

	// wp_safe_redirect( home_url( $_SERVER['REQUEST_URI'] ) );

	// wp_safe_redirect( home_url( $_SERVER['REQUEST_URI'] . '&wcpls=ok' ) );
...
function woocommerce_maybe_add_multiple_products_to_cart() {
    ...
    if( isset( $_REQUEST['add_more_to_cart'] ) && intval( $_REQUEST['add_more_to_cart'] ) == 1 ){
        ...
        if( count( $product_ids ) == $success && $success != 0 ){
            ...
            $message = sprintf( '<a href="%s" tabindex="1" class="button wc-forward">%s</a> %s', esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'View cart', 'mpc' ), esc_html( $added_text ) );
