...
function wmc_admin_options( $settings, $current_section ) {
  ...
  if ( $current_section == 'wmc' ) {
    ...
    $wmc_settings[] = array( 'type' => 'sectionend', 'id' => 'wmc' );
    return $wmc_settings;
    /**
...
if( !function_exists('mpc_extract_atts') ) {
  function mpc_extract_atts( $atts ){
    ...
    // common
    if( $atts['limit'] > 50 || $atts['limit'] < 0 ) $atts['limit'] = 50;
    $args = array(
      'post_type'         => 'product',
      'post_status'       => 'publish',
    ...
    if( in_array( $atts['orderby'], array( 'title', 'date' ) ) ) $args['orderby'] = ( $atts['orderby'] != '' ? $atts['orderby'] : 'date' );
    if( isset( $atts['order'] ) ) $args['order'] = ( $atts['order'] != '' ? strtoupper( $atts['order'] ) : 'DESC' );
    ...
    if( $atts['ids'] != '' && $atts['cats'] == '' && $atts['type'] == 'all' ){
    	...
    	if( $atts['orderby'] != '' || $atts['order'] != '' ){
    		...
    		if( isset( $ids ) && count( $ids ) > 0 ){
    			foreach( $ids as $id ){
    				$post = get_post( $id );
    				if( isset( $post ) ) array_push( $validated, $id );
    			}
    		}
      	return $validated;
    	}
    ...
    elseif( $atts['ids'] == '' && $atts['cats'] != '' && $atts['type'] == 'all' ){
    	...
    	if( isset( $termids ) && count( $termids ) > 0 ){
    		foreach( $termids as $id ){
    			$term = get_term( $id );
    			if( isset( $term ) ) array_push( $validated, $id );
    		}
    	}
	    $args['tax_query'] = array(
        array(
    			...
    			foreach( $all_posts as $id ){
    				...
    				if( isset( $terms ) ){
    					foreach( $terms as $term ){
    						if( $term->term_id == $termid && !in_array( $id, $sorted ) ) array_push( $sorted, $id );
    					}
    				}
    			}
    		}
    ...
    elseif( $atts['ids'] == '' && $atts['cats'] == '' && $atts['type'] != 'all' ){
    	...
    	if( in_array( $atts['type'], array( 'simple', 'variable' ) ) ){
  	    ...
  	    while( $posts->have_posts() ) : $posts->the_post();
	        ...
	        if( isset( $_product ) && $_product->is_type( $atts['type'] ) ) array_push( $ids, $id );
  	    endwhile;
    	}
    	wp_reset_postdata();
    	return $ids;
    ...
    elseif( $atts['ids'] == '' && $atts['cats'] != '' && $atts['type'] != 'all' ){
    	...
    	while ( $posts->have_posts() ) {
  	    ...
  	    if( isset( $_product ) && $_product->is_type( $atts['type'] ) ) array_push( $all_posts, get_the_ID() );
    	}
    	wp_reset_postdata();
    	$sorted = array();
    	if( isset( $all_posts ) && count( $all_posts ) > 0 ){
...
if( !function_exists('mpc_multicart_shortcode') ) {
  function mpc_multicart_shortcode( $atts ) {
    ...
    else include( mpc_locate_template( WMC_DIR . '/templates/error.php' ) );
...
function mpc_go_to_designated_page( $url = '' ){
  ...
  else wp_safe_redirect( wc_get_cart_url() );
	exit;
}
/**
...
function mpc_get_variation_id( $at_key, $pid ){
    ...
    foreach ( $vs as $v ) {
      ...
      foreach( $at_key as $key => $value ){
        ...
        if( isset($saved_value) && $saved_value == $value ) $count++;
      }
      if( count( $at_key ) == $count ) $flag = 100;
      if( $flag == 100 ) $variation_id = $v->ID;
      else
        if( $count > 0 && $count < count( $at_key ) ){
          foreach( $at_key as $key => $value ){
            $saved_value = get_post_meta( $v->ID, $key, TRUE );
            if( isset($saved_value) && $saved_value == $value ) $variation_id = $v->ID;
          }
        }
    }
	return $variation_id;
...
function woocommerce_maybe_add_multiple_products_to_cart() {
   ...
   if( isset( $_REQUEST['add_more_to_cart'] ) && intval( $_REQUEST['add_more_to_cart'] ) == 1 ){
     ...
     if ( count( $product_ids ) == 0 ) {
       if ( function_exists( 'wc_add_notice' ) ) wc_add_notice( __( 'Please select one or more products.', 'woocommerce' ), 'error' );
     ...
     foreach( $product_ids as $pid ){
       ...
       if( $_product->is_type( 'simple' ) ){
        if ( false !== WC()->cart->add_to_cart( $pid, $quantity ) ) $success++;
         ...
         if( $saved == false ) mpc_go_to_designated_page();
       }
 ...
 if( !function_exists( 'mpc_check_if_variation_exists' ) ) {
 	function mpc_check_if_variation_exists( $ids ) {
 		$flag = false;
 		if( isset( $ids ) && count( $ids ) > 0 ) : foreach( $ids as $id ) :
 			$_product = wc_get_product( $id );
 			if( $_product->is_type( 'grouped' ) ) continue;
 			if( $_product->is_type( 'variable' ) ) $flag = true;
 		endforeach; endif;
 		return $flag;
 	}
 }
