...
if ( ! class_exists( 'MPC_Template' ) ) {
	...
	class MPC_Template {
		...
		public function table_check_all() {
			...
			// hide if checkbox column is hidden.
			if ( ! isset( $this->data['options']['mpc_add_to_cart_checkbox'] ) || ! $this->data['options']['mpc_add_to_cart_checkbox'] ) {
				return;
			}
		...
		public function table_cart_button() {
			...
			?>
			<input type="submit" class="mpc-add-to-cart button alt wc-forward" name="proceed" value="<?php echo esc_html( $this->data['labels']['wmc_button_text'] ); ?>" />
			<?php
		}
...
$mpc_template__ = new MPC_Template();
$mpc_template__->init();
