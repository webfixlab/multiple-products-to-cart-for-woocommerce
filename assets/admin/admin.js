	...
	$(document).ready(function () {
		...
		function column_sorting() {
			...
			$('#active-mpc-columns').find('li').each(function () {
				if (sequence.length == 0) sequence += $(this).data('meta_key');
				else sequence += ',' + $(this).data('meta_key');
			});
		...
		$(document).on('keyup', function (e) {
			if (e.keyCode == 27) $('body').find('#mpcpop').hide(); // esc key pressed.
		});
		...
		$(window).on('scroll', function () {
			...
			if ($(window).scrollTop() > stickyOffset) $('.mpcdp_sidebar_tabs').addClass('is-affixed');
			else if ($('.mpcdp_sidebar_tabs').hasClass('is-affixed')) $('.mpcdp_sidebar_tabs').removeClass('is-affixed');
		});
		...
		function update_sc_input(wrap, values) {
		...
		$('body').find('.mpc-sc-itembox').each(function () {
			...
			type.passedElement.element.addEventListener('addItem', function (event) {
				update_sc_input($(this).closest('.choicesdp'), type.getValue(true));
			});
		...
		$(document).on('click', '#mpc-export', function (e) {
			e.preventDefault();
			const btn = $(this);
